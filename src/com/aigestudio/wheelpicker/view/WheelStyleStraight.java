...
/**
 * 滚轮默认直的样式实现 Default style implement of WheelView in Straight
 *
...
class WheelStyleStraight extends AbstractWheelStyle {
	...
	@Override
	void computeWheel() {
		width = direction.computeStraightWidth(view.itemCount, view.itemSpace, maxTextWidth, maxTextHeight);
		height = direction.computeStraightHeight(view.itemCount, view.itemSpace, maxTextWidth, maxTextHeight);
	...
	void drawItems(Canvas canvas) {
		for (int i = -view.itemIndex; i < view.data.size() - view.itemIndex; i++) {
			...
			canvas.drawText(view.data.get(i + view.itemIndex), centerX, centerTextY + curUnit, paint);
			canvas.restore();
		}
	}
	...
	void onTouchEventMove(MotionEvent event) {
		...
		if (Math.abs(distanceSingleMove) < WheelCons.TOUCH_DISTANCE_MINIMUM)
			return;
		if (Math.abs(distanceSingleMove) >= width)
			return;
