...
/**
 * 滚轮弯曲样式实现 Curve style implement of WheelView
 *
 * @author AigeStudio 2015-12-03
 * @author AigeStudio 2015-12-08 修复当前Item静止后跳动bug
 * @version 1.0.0 beta
 */
class WheelStyleCurved extends AbstractWheelStyle {
	...
	void drawItems(Canvas canvas) {
		for (int i = -view.itemIndex; i < view.data.size() - view.itemIndex; i++) {
			...
			canvas.drawText(view.data.get(i + view.itemIndex), centerX, centerTextY + space, paint);
			canvas.restore();
		}
	}
	...
	void onTouchEventMove(MotionEvent event) {
		...
		if (Math.abs(distanceSingleMove) < WheelCons.TOUCH_DISTANCE_MINIMUM)
			return;
		if (Math.abs(distanceSingleMove) >= radius)
			return;
